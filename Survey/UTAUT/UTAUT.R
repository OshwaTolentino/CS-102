...
#Factoring the elements of every column in Performance Expectancy and change it in numeric value
...
#Calculate the mean and standard deviation of Performance Expectancy rounded to 2 decimal places
...
#Creating a data frame for Performance Expectancy data
PE_data <- data.frame(
  Questions = c("Do you find using Digital payment useful?", "Do you think using Digital payment method will help you manage your money better?", "Do you believe that Digital payment method will make your financial transaction easier?", "Do you think Digital payment method will make it easier for you to keep track of your spending?"),
  Description = c("Performance Expectance", "Performance Expectance", "Performance Expectance", "Perforamce Expectancy"),
  Mean = c(PE1_mean, PE2_mean, PE3_mean, PE4_mean),
  SD = c(PE1_sd, PE2_sd, PE3_sd, PE4_sd)
)

# Save the data frame of Performance Expectancy as a CSV file
write.csv(PE_data, "PE.csv", row.names = FALSE)




#Factoring the elements of every column in Effort Expectancy and change it in numeric value
#Getting the mean and standard deviation
...
#Rounding the value of mean and standard deviation into 2 decimal places
...
#Creating a data frame for Performance Expectancy data
EE_data <- data.frame(
  Questions = c("Do you find Digital payment method easy to use and understand?", "Do you find digital payment platforms user-friendly and efficient for making transactions?", "Do you find the process of digital payment platforms with other financial tools and services beneficial?", "Do you find digital payment method convenient to use?"),
  Description = c("Effort Expectancy", "Effort Expectancy", "Effort Expectancy", "Effort Expectancy"),
  Mean = c(EE1_mean, EE2_mean, EE3_mean, EE4_mean),
  SD = c(EE1_sd, EE2_sd, EE3_sd, EE4_sd)
)

# Save the data frame of Effort Expectancy as a CSV file
write.csv(EE_data, "EE.csv", row.names = FALSE)




#Factoring the elements of every column in Social Influence and change it in numeric value
#Getting the mean and standard deviation
...
#Creating a data frame for Social Influence data
SI_data <- data.frame(
  Questions = c("Do your friends or family members encourage you to use digital payment methods?", "Are you influenced by the behavior of others when it comes to adopting digital payment technologies?", "Do you feel pressure from social circles to use digital payment platforms?"),
  Description = c("Social Influence", "Social Influence", "Social Influence"),
  Mean = c(SI1_mean, SI2_mean, SI3_mean),
  SD = c(SI1_sd, SI2_sd, SI3_sd)
)

# Save the data frame of Social Influence as a CSV file
write.csv(SI_data, "SI.csv", row.names = FALSE)

#Factoring the elements of every column in Facilitating Condition and change it in numeric value
#Getting the mean and standard deviation
...
#Creating a data frame for Facilitating Condition data
FC_data <- data.frame(
  Questions = c("How easy is it for you to access digital payment platforms like GCash, PayMaya, PayPal, etc.?", "Have you faced any problems using digital payment platforms?", "What features would you like digital payment platforms to have to make them easier to use?"),
  Description = c("Facilitating Condition", "Facilitating Condition", "Facilitating Condition"),
  Mean = c(FC1_mean, FC2_mean, FC3_mean),
  SD = c(FC1_sd, FC2_sd, FC3_sd)
)

# Save the data frame of Facilitating Condition as a CSV file
write.csv(FC_data, "FC.csv", row.names = FALSE)


#Reading each CSV Files
...
#Data frame for Performance Expectancy summary
...
# Save PE merged summary data frame as a CSV file
write.csv(PE_summary, "PE_Summary.csv", row.names = FALSE)



#Data frame for Facilitating Condition summary
...
#Data frame for Effort Expectancy summary
...
#Data frame for Social Influence summary
...
#Adding average mean and sd column
read_merged_data <- read_merged_data %>%
  mutate(Average_Mean = c(PE_merged_mean, PE_merged_mean, PE_merged_mean, PE_merged_mean, EE_merged_mean, EE_merged_mean, EE_merged_mean, EE_merged_mean, SI_merged_mean, SI_merged_mean, SI_merged_mean, FC_merged_mean, FC_merged_mean, FC_merged_mean),
        Average_Sd = c(PE_merged_sd, PE_merged_sd, PE_merged_sd, PE_merged_sd, EE_merged_sd, EE_merged_sd, EE_merged_sd, EE_merged_sd, SI_merged_sd, SI_merged_sd, SI_merged_sd, FC_merged_sd, FC_merged_sd, FC_merged_sd))

#Creating a data frame
read_merged_data <- read_csv("/cloud/project/Survey/UTAUT/CSV Files/MergedData.csv")
write.csv(read_merged_data, "Final_Merged.csv", row.names = FALSE)

#Displaying Data frame using kable function
kable(read_merged_data, caption = "Understanding the Role of Digital Wallets")
